<template>
  <div class="dailyChallenge">
    <pre style="text-align: left" @click="text = null">{{ text }}</pre>

    <div style="display: flex">
      <pre style="width: 100%; text-align: left">{{ from }}</pre>
      <pre style="width: 100%; text-align: left">{{ to }}</pre>
    </div>

    <div style="text-align: left; max-width: 800px">
      <div
        v-for="(day, index) in this.dailyChallengeData"
        :key="index"
        @click="consol.lag('hi')"
      >
        <div class="day">
...
export default defineComponent({
  ...
  computed: {
    currentDay() {
      return this.dailyChallengeData?.[this.dailyChallengeData?.length - 1];
    },
  },
  async mounted() {
    if (navigator.userAgent.match(/(iPhone|iPod|iPad|Android|BlackBerry)/)) {
      document.addEventListener(
        ...
        async () => {
          ...
          this.dailyChallengeData = await fileToJson(text);
        },
        false
      );
    } else {
  ...
  methods: {
    ...
    async inputTag({ tag, text }) {
      if (!this.dailyChallengeData?.length) return;

      let days = this.dailyChallengeData;
      ...
      fileStorage.saveFile(
        "/Download",
        "daily-challenge.xml",
        await jsonToFile(this.dailyChallengeData)
      );
    },
    async readFile() {
